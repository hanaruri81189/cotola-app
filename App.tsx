...
import React, { useState, useCallback, ChangeEvent, FormEvent, useEffect, useRef, useLayoutEffect } from 'react';
import { GoogleGenAI, Chat, GenerateContentResponse, Content } from "@google/genai";
import { GenerationParams, SNSType, Tone, GroundingChunk, Candidate } from './types';
import { TONE_OPTIONS, SNS_OPTIONS, SNS_MAX_CHARS, GEMINI_MODEL_NAME } from './constants';
import { generateContent as callGeminiApi } from './services/geminiService';
...
interface SelectFieldProps<T extends string | number> { // Allow T to be number for storyPages
  label: string;
  id: string;
  value: T;
...
const BRAILLE_BLANK = '\u2800'; // Braille Pattern Blank
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      // Per Gemini API guidelines, API key MUST be obtained from process.env.API_KEY
      const apiKey = process.env.API_KEY;
      if (!apiKey) {
        setError("APIキーが設定されていません。Vercelなどのホスティング環境で、環境変数名を「API_KEY」として正しいAPIキー値を設定してください。");
        console.warn("API_KEY environment variable is not set. Cotola™ may not function fully. Ensure it's set in your deployment environment (e.g., Vercel) with the name API_KEY.");
        return;
      }
      ...
      if (e instanceof Error && e.message.includes('API key not found')) {
         setError("APIキーが見つかりません。環境変数 API_KEY が正しく設定されているか確認してください。");
      } else {
        setError("AIサービスの初期化に失敗しました。ページを再読み込みするか、管理者にお問い合わせください。");
      }
    }
  }, []);
  ...
  const handleSubmit = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!aiInstanceRef.current) {
        setError("AIサービスが利用できません。APIキーが正しく設定されているか確認し、ページを再読み込みしてください。Vercelなどの環境では、環境変数「API_KEY」が設定されていることを確認してください。");
        setIsLoading(false);
        return;
    }
    setIsLoading(true);
    ...
    try {
      const result = await callGeminiApi(aiInstanceRef.current, params);
      ...
      if (finalText && aiInstanceRef.current) {
        ...
        if (selectedSnsType === SNSType.INSTAGRAM_STORIES) {
  ...
  const handleChatSubmit = useCallback(async () => {
    ...
    if (!chatSessionRef.current) {
        setError("チャットセッションが初期化されていません。まず文章を生成してください。");
        return;
    }
    ...
    try {
        const unformattedGeneratedText = (selectedSnsType === SNSType.INSTAGRAM || selectedSnsType === SNSType.FACEBOOK)
                                        ? unformatTextFromInstagramFacebook(generatedText)
                                        : generatedText;

        const messageToSend = `現在の文章は以下の通りです。これを元に、あなたの指示を反映させてください。\n\n現在の文章:\n「${unformattedGeneratedText}」\n\n追加の指示:\n「${chatInstruction}」`;
        
        const responseStream = await chatSessionRef.current.sendMessageStream({ message: messageToSend });
        
        let newTextChunks: string[] = [];
        for await (const chunk of responseStream) {
            if (chunk.text) {
                 newTextChunks.push(chunk.text);
            }
        }
        let newText = newTextChunks.join('');
  ...
  }, [chatInstruction, generatedText, selectedSnsType]);
    ...
    <div className="min-h-screen bg-rose-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        {!isLoading && !isChatLoading && (
          <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl space-y-6 ring-1 ring-gray-200" aria-labelledby="form-heading">
            ...
            <Button type="submit" disabled={isLoading || !episode.trim() || !feelings.trim() || !aiInstanceRef.current} className="w-full text-lg py-3" aria-label={isLoading ? '文章を生成中' : '文章を生成する'}>
          ...
          <section className="mt-8 bg-white p-6 sm:p-8 rounded-xl shadow-2xl ring-1 ring-gray-200" aria-labelledby="generated-content-heading">
            ...
            <div className="mt-6 pt-6 border-t border-gray-200">
              ...
              <Button
                ...
                disabled={isChatLoading || !chatInstruction.trim() || !chatSessionRef.current}
       ...
       <footer className="mt-12 text-center text-sm text-gray-500">
        <p>&copy; {new Date().getFullYear()} hanaruri. Powered by Gemini API.</p>
